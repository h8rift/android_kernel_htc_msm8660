...
static short palm_rejection_enable;
...
struct ntg_tsData {
	...
	struct workqueue_struct *ntg_wq_palmRej;
	struct work_struct 	ntg_work_palmRej;
	...
	uint8_t palm_rejection_attr;
	uint8_t palm_rejection_setting;
	uint8_t palm_rejection_pendding;
	int	(*power)(int on);
	int	(*lcm_ts_power)(int on);
	bool esdFlag;
...
static ssize_t spiDuplex_Lock(struct spi_device *dev, char *txbuf, char *rxbuf, size_t len)
{
	struct spi_msg msg;
	int err = 0, loop_i;
	if (!dev)
...
DEVICE_ATTR(event_google, 0644, show_event_google, set_event_google);
...
static ssize_t set_palm_rejection(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	int palm_setting = -1;
	struct ntg_tsData *ts;
	static uint8_t prt_info;
	ts = private_ts;

	palm_setting = simple_strtoul(buf, NULL, 10);

	if (ts->palm_rejection_attr) {
		ts->palm_rejection_setting = palm_setting;
		if (ts->unlock_state == 2 || ts->unlock_state == 0)
			queue_work(ts->ntg_wq_palmRej, &ts->ntg_work_palmRej);
		else if (ts->unlock_state == 1) {
			ts->palm_rejection_pendding = 1;
			pr_info("[ts]palm_rejection_pendding\n");
		}
	} else {
		if (!prt_info) {
			pr_info("[ts]set palm_rejection don't support\n");
			prt_info = true;
		}
	}
...
static ssize_t show_palm_rejection(struct device *dev, struct device_attribute *attr, char *buf)
{
	pr_info("[ts]palm_rejjection: %d\n", palm_rejection_enable);
	return sprintf(buf, "palm_rejection %d\n", palm_rejection_enable);
}
DEVICE_ATTR(palm_rejection, 0644, show_palm_rejection, set_palm_rejection);

static ssize_t ntrig_unlock_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk(KERN_INFO "[ts]unlock change to %d\n", unlock);

	ts->unlock_state = unlock;

	if (ts->unlock_state == 2) {
		if (ts->palm_rejection_attr && ts->palm_rejection_pendding) {
			queue_work(ts->ntg_wq_palmRej, &ts->ntg_work_palmRej);
			ts->palm_rejection_pendding = 0;
		}
	}
...
static int ntrig_touch_sysfs_init(void)
{
	...
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_palm_rejection.attr);
	if (ret) {
		pr_err("[tp_ntg_err]sysfs_create_file failed\n");
		return ret;
	}
...
static void ntrig_touch_sysfs_remove(void)
{
	...
	sysfs_remove_file(android_touch_kobj, &dev_attr_palm_rejection.attr);
...
static int ntrig_debug_agent_copy(struct ntg_tsData *ntg, char *buf, int index)
{
	...
	memcpy(&g_data_uart.data[index], &buf[10], ntg->touchEvnt->header.length - 6);
	if (TSLog & Dbg_L4) {
	...
	if (temp_header.channel == 0x30) {
		if (temp_header.flag == 0 && pre_flag == 0) {/*only one packet*/
			...
			if (pre_flag == 0 || (temp_header.flag >= 0 &&/*check flag*/
			    ...
			    (temp_header.function == g_data_uart.header.function))) {/*check function*/
...
static ssize_t ntrig_agent_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	unsigned long size;
	struct ntg_tsData *ntg;
...
{
	bool fCheckSum = true;
	int i, nIndex = 0, loop_i, contact_count, DTime = 0, region = 0;
			...
			case 0x20:
				...
				case 0x61:
					if ((buf[nIndex+6] == 0x7E && buf[nIndex+11] == 0x81) ||
						...
						memcpy((void *)&ntg->touchEvnt->DfuStCmd.dfuHeader, &buf[nIndex + 6],
						       sizeof(ntg_CmdDfuHeader));
...
static void ntrig_ts_work_palmRej(struct work_struct *work)
{
	int i = 0;
	struct ntg_tsData *ts;
	char spiPalmTx[NTRIG_PACKET_SIZE];
	ts = container_of(work, struct ntg_tsData, ntg_work_palmRej);

	memset(spiPalmTx, 0xAA, NTRIG_PACKET_SIZE);

	switch (ts->palm_rejection_setting) {
	case 0:/*Plam Rejection Off*/
		ntgFW_mode[28] = 0x02;
		ntgFW_mode[29] = 0x0A;
		ntgFW_mode[31] = 0x00;
		break;
	case 1:/*Plam Rejection On*/
		ntgFW_mode[28] = 0x02;
		ntgFW_mode[29] = 0x0A;
		ntgFW_mode[31] = 0x01;
		break;
	default:
		pr_err("[ts]set_palm_rejection value not allow\n");
	}

	if ((uint16_t)ntg_fwver >= ts->fwtwozero) {
		ntgFW_mode[sizeof(ntgFW_mode)-1] = ntg_checksumCal(&ntgFW_mode[14], 21);
		memcpy(spiPalmTx, ntgFW_mode, sizeof(ntgFW_mode));
	} else {
		ntgFW_mode[9] = 0x1A;
		ntgFW_mode[17] = 0x14;
		ntgFW_mode[sizeof(ntgFW_mode)-3] = ntg_checksumCal(&ntgFW_mode[14], 19);
		memcpy(spiPalmTx, ntgFW_mode, (sizeof(ntgFW_mode)-2));
	}
	fPalmRej = false;
	do {
		ntg_spi_transfer(ts, spiPalmTx);
		msleep(150);
		i++;
	} while (!fPalmRej && i < 4);

	if (fPalmRej) {
		pr_info("[ts] set palm rejection : %d\n", ts->palm_rejection_setting);
	} else {
		pr_info("[ts] set palm rejection fail\n");
	}
}
...
static int ntrig_ts_probe(struct spi_device *dev)
{
	...
	if (pdata->pmicgpio_config != NULL) {
		pr_info("[ts] pmicgpio_config\n");
		pdata->pmicgpio_config();
	}
	...
	if (pdata) {
		if (pdata->fw_version)
			while ((uint16_t)ntg_fwver < pdata->fw_version)
				pdata++;
		ntg_ts->power = pdata->power;
		if (pdata->lcm_ts_power)
			ntg_ts->lcm_ts_power = pdata->lcm_ts_power;
	}
	...
	ntg_ts->palm_rejection_attr = pdata->palm_rejection_attr;
...
/*#endif*/
	...
	if (ntg_ts->palm_rejection_attr) {
		ntg_ts->ntg_wq_palmRej = create_singlethread_workqueue("ntrig_wq_palmRej");
		if (!ntg_ts->ntg_wq_palmRej) {
			pr_err("[tp_ntg_err]allocate ntg_tsdata failed \n");
			ret = -ENOMEM;
			goto err_create_wq_failed;
		}
		INIT_WORK(&ntg_ts->ntg_work_palmRej, ntrig_ts_work_palmRej);
	}
...
err_input_dev_alloc_failed:
	...
	destroy_workqueue(ntg_ts->ntg_wq_palmRej);
...
static int ntrig_ts_suspend(struct spi_device *dev, pm_message_t mesg)
{
	...
	if (ret)
		enable_irq(ts->spiDev->irq);
	if (ts->palm_rejection_attr)
		ret = cancel_work_sync(&ts->ntg_work_palmRej);
